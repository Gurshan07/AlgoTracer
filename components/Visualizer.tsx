...
import { AnalysisResult, Step, StepState } from '../types';
import { ArrowRight, Box, Braces, Layers } from 'lucide-react';
...
export const Visualizer: React.FC<VisualizerProps> = ({ analysis, currentStepIndex }) => {
  ...
  // Merge initial state with current step state to ensure we have values
  // Logic: The API step state might only return *changes*. 
  // Ideally the prompt asks for state, but let's be robust.
  // Actually, the prompt schema implies full state in "state". 
  // We will assume the API returns the snapshot.
  
  const variables = currentStep?.state?.variables || {};
  const arrays = currentStep?.state?.dataStructures?.arrays || {};

  // Helper to find which indices are being accessed based on variable values that look like indices
  // This is a heuristic since the API might not explicitly say "highlight index 2" in a structured way 
  // other than the description.
  const activeIndices: Record<string, number[]> = {};
  
  // Try to infer active indices from variables named i, j, k, left, right, mid
  Object.entries(variables).forEach(([key, value]) => {
    if (['i', 'j', 'k', 'l', 'r', 'mid', 'low', 'high', 'p', 'q'].includes(key) && typeof value === 'number') {
      // Find which array this might apply to. For now, apply to all arrays for visualization simplicity
      // or check if the description mentions specific array.
  ...
  return (
    <div className="flex flex-col h-full gap-6 overflow-y-auto pr-2">
      ...
      {/* Step Description */}
      <div className="bg-gray-850 p-4 rounded-lg border border-gray-700 shadow-sm animate-fade-in">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
          <Box size={16} /> Data Structures
        </h3>
        
        {Object.keys(arrays).length === 0 && (
           <div className="text-gray-500 italic text-sm p-4 border border-dashed border-gray-700 rounded">No arrays detected</div>
        )}
          ...
          <div key={name} className="bg-gray-850 p-4 rounded-lg border border-gray-700 overflow-x-auto">
            ...
            <div className="flex items-end gap-2 pb-6 min-w-max">
              {Array.isArray(data) && data.map((val, idx) => {
                 ...
                 const variablePointer = Object.entries(variables).find(([k, v]) => 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-700 font-mono">
                {Object.entries(variables).map(([key, val]) => (
                   // Skip array variables in this list as they are shown above
                   !Object.keys(arrays).includes(key) && (
                ...
                {Object.keys(variables).filter(k => !Object.keys(arrays).includes(k)).length === 0 && (
                  <tr>
        ...
        <div className="space-y-6">
            ...
            {/* Call Stack Placeholder (Basic) */}
             {/* 
                Call stack tracking is complex to visualize perfectly without a deep recursion tree. 
                We display the current function scope.
             */}
            <div className="space-y-2">
               <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                <Layers size={16} /> Scope
              </h3>
